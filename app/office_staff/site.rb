  ...
  action_item 'Billing', only: [:show, :edit] do
    if site.contract.present? && site.project.present? && site.production.present?
      if site.billing.present?
        link_to 'Billing', office_staff_site_billing_url(site, site.billing)
      else
        link_to 'Create Billing', new_office_staff_site_billing_url(site)
      end
    end
  end
    ...
    f.inputs 'Address' do
      f.fields_for :address do |af|
        if params[:customer_id]
          ...
          af.input :customer, required: true, as: :autocomplete, url: autocomplete_site_customer_office_staff_sites_path, input_html: { id_element: '.customer_id_element', value: af.object.customer.try(:autocomplete_display_value), placeholder: 'Search Email' }
