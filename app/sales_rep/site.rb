ActiveAdmin.register Site, namespace: 'sales_rep' do
  menu priority: 3
  ...
  action_item 'Contract', only: [:show, :edit] do
    if site.contract.present?
      link_to 'Contract', sales_rep_site_contract_url(site, site.contract)
    else
      link_to 'Add Contract', new_sales_rep_site_contract_url(site)
    end
  end
  ...
  action_item 'Project', only: [:show, :edit] do
    if site.contract.present?
      if site.project.present?
        link_to 'Project', sales_rep_site_project_url(site, site.project)
      else
        link_to 'Create Project', new_sales_rep_site_project_url(site)
      end
    end
  end

  action_item 'Docs', only: [:show, :edit] do
    link_to 'Docs', sales_rep_site_documents_url(site)
  end
  ...
  controller do
    def autocomplete_site_customer
      @customers = Customer.accessible_by(current_ability, :read).includes(:primary_phone_number).joins(:phone_numbers).where('firstname like :term OR lastname like :term OR phone_numbers.number like :term', term: "%#{params[:term].downcase}%").uniq.order(firstname: :asc).limit(10).collect do |cust|
    ...
    f.inputs 'Address' do
      f.fields_for :address do |af|
        if params[:customer_id]
          ...
          af.input :customer, required: true, as: :autocomplete, url: autocomplete_site_customer_sales_rep_sites_path, input_html: { id_element: '.customer_id_element', value: af.object.customer.try(:autocomplete_display_value), placeholder: 'Search Email' }
